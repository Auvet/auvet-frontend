...
const funcClinicaRepo = new FuncionarioClinicaRepository();
...
export function FuncionarioCreate() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const cargoFinal = form.cargo === 'Outros' ? cargoOutros : form.cargo;
      
      if (!cargoFinal) {
        setMessage('Cargo é obrigatório.');
        setLoading(false);
        return;
      }
      
      await authService.registerFuncionario({
        cpf: cleanCpfValue,
        nome: form.nome,
        email: form.email,
        senha: form.senha,
      ...
      const cnpj = getCnpj();
      if (!cnpj) {
        setMessage('Funcionário cadastrado. Atenção: CNPJ da clínica não encontrado para vincular.');
      } else {
        await funcClinicaRepo.createRelation(cleanCpfValue, cnpj);
        setMessage('Funcionário cadastrado e vinculado à clínica com sucesso!');
      }
      setForm({ cpf: '', nome: '', email: '', senha: '', cargo: '', registroProfissional: '' });
      setCargoOutros('');
    } catch (err) {
    ...
    <Box style={{ padding: '40px' }}>
      <Box mb="xl">
        ...
        <Text size="lg" c="dimmed">
          Crie um novo funcionário e vincule à clínica atual
        </Text>
      </Box>
      ...
      <form onSubmit={handleSubmit}>
        <Grid>
          ...
          <Grid.Col span={{ base: 12, md: 6 }}>
            <TextInput label="Registro Profissional" value={form.registroProfissional} onChange={(e) => setForm({ ...form, registroProfissional: e.currentTarget.value })} />
          </Grid.Col>
        </Grid>
